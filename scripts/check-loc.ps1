# LOC budget (PowerShell) — tools+scripts capped; docs/SECURITY reported only
...
$aux = $tools + $scripts
$total = $client + $server + $aux + $docsSec
...
if ($aux -gt 350) { Write-Host "FAIL: tools+scripts $aux > 350"; $fail = $true }
